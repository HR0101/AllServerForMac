...
| `Features/MediaAnalysis/Models/VariantVideoDetector.swift` | 尺＋複数フレームの知覚ハッシュによる差分動画の検出 |
...
実測（4K/60fps の同一シリーズ 10 本）での距離の目安:

| 関係 | 平均距離 |
|---|---|
| 衣装・状態だけが違う差分どうし | 2 〜 9 |
| 同じ動きだがキャラクターが違う | 14 〜 19 |
| まったく別の動画 | 26 以上 |

既定のしきい値 **10** はこの谷間に置いています。画面下のスライダーでその場で締めたり緩めたりできます（緩めると、同じ動きの別キャラまで 1 つの束に混ざります）。
...
- **手動切り替え**：数字キー **1〜9** でその番号へ直接、`N` / `B` で次・前、`V` でランダム。手で切り替えたときも次の自動切り替えまでの時間は測り直します。
- **その他のキー**：`A` 自動切り替えの入/切、`−` / `+` 間隔を 0.5 秒ずつ増減、`T` 下の操作パネルの表示/非表示、`?` ショートカット一覧、`Esc` 終了。再生・シーク・閉じるは通常の動画プレイヤーと同じキーです（設定 →「ショートカットキー」で変更可）。
