...
- **BGMの拍に合わせる**：自動切り替えをオンにした状態で有効にすると，低域を重視した短時間エネルギーからBGMの拍候補をバックグラウンド検出し，通常の切り替え予定時刻に最も近い拍へ最大0.75秒だけタイミングを調整します．通常差分では動画時刻をそのまま使い，一部一致差分では拍時刻を共通タイムラインへ変換します．拍とは音楽でリズムを刻むタイミングです．音声なし，拍解析の失敗，近くに拍がない場合は従来のランダム間隔へ自動的に戻ります．BGMだけを音源分離する処理ではないため，強い効果音を拍として拾う場合があります．初期値はオフで，選択状態は次回起動後も保持します．
