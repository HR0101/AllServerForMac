...
サイドバーの「解析」→「シーン抽出」から利用します．動画を開いて「候補を解析」を押すと，音声の100〜1500Hz帯域，映像の輝度・エッジ差分，freeze・fade・dissolve・編集遷移をバックグラウンドで解析します．解析はキャンセル可能で，動画フレームそのものは保持せず，時系列特徴量だけをメモリに残します．
